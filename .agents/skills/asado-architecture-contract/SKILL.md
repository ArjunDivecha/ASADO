...
You are a zero-context Codex (or human) engineer about to touch ASADO. ASADO is a data-collection
...
detail): `AGENTS.md` and `AGENTS.md` at the repo root; `openwiki/quickstart.md` for navigation;
...
schema `asado`). This is documented in `AGENTS.md` under "setup_duckdb.py deletes and recreates".
...
allows one read-write process OR many read-only processes; a single stray holder (a Codex
Desktop analysis-sandbox kernel under `~/.Codex-science/` was the real culprit) blocks every
...
default `.Codex-science`, `DEFAULT_KILLABLE_PATTERNS` at :84) SIGTERMs it, escalating to
