...
You are (probably) a fresh Codex session with zero prior context, operating
...
- Do NOT open a `.duckdb` file interactively (no `duckdb Data/asado.duckdb`, no
  ...
  `.Codex-science` lock holders, 300s budget, fails loudly). Several paths are
...
- The repo's own permission allowlist (`.Codex/settings.local.json`) only
...
| `AGENTS.md` | TRUST (LIVE) | Project rules + a dated `## Current state (YYYY-MM-DD)` section. | `git log -1 --format=%ci -- AGENTS.md`; also check `git status` for an uncommitted working-copy edit |
...
2. `AGENTS.md` → the `## Current state (YYYY-MM-DD)` section — the human-curated
...
authority (`AGENTS.md` / `AGENTS.md`) or the ledgers, trust the live authority
