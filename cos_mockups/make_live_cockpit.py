...
function tripRow(r,showC){return `<tr onclick="openCountry('${qs(r.country)}')">
  ...
  <td class="num">${r.triptych_url?`<a href="${esc(r.triptych_url)}" target="_blank" rel="noopener" onclick="return tripOpen(event,this.href,'${qs(r.factor)} · ${qs(r.country)} · ${r.horizon_months}M')" style="color:var(--teal);text-decoration:none" title="open the Triptych (popup; new tab if popups blocked)">⧉</a>`:"—"}</td></tr>`}
