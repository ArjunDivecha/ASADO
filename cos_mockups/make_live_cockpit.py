...
function tripRow(r,showC){const label=`${r.factor} · ${r.country} · ${r.horizon_months}M`;
  const rowAction=r.triptych_url
    ?`return tripOpen(event,'${qs(r.triptych_url)}','${qs(label)}')`
    :`openCountry('${qs(r.country)}')`;
  return `<tr data-triptych-run="${r.triptych_url?'graph':'country'}" onclick="${rowAction}" title="${r.triptych_url?'Open the actual Triptych graph':'Open the country panel'}">
  ...
  <td class="num">${r.triptych_url?`<a href="${esc(r.triptych_url)}" target="_blank" rel="noopener" aria-label="View Triptych graph for ${esc(label)}" onclick="return tripOpen(event,this.href,'${qs(label)}')" style="color:var(--teal);text-decoration:none;white-space:nowrap;font-weight:600" title="Open the actual Triptych graph">View graph&nbsp;⧉</a>`:"—"}</td></tr>`}

/* Triptych popup (2026-07-11): in-page modal hosting the actual Triptych visual
   tool in an iframe (triptych-one.vercel.app now sends frame-ancestors *).
   Falls back to the anchor's target=_blank navigation if JS is disabled. */
function tripOpen(ev,url,label){ev.preventDefault();ev.stopPropagation();
  let m=document.getElementById("tripmodal");
  if(!m){m=document.createElement("div");m.id="tripmodal";document.body.appendChild(m)}
  m.innerHTML=`<div onclick="tripClose()" style="position:fixed;inset:0;background:rgba(24,22,18,.55);z-index:200"></div>
  <div role="dialog" aria-label="Triptych" style="position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);z-index:201;width:min(1320px,96vw);height:min(880px,94vh);background:#FDFCF9;border:1px solid #D8D2C4;border-radius:10px;box-shadow:0 18px 60px rgba(0,0,0,.35);display:flex;flex-direction:column;overflow:hidden">
    <div style="display:flex;align-items:center;gap:10px;padding:9px 14px;border-bottom:1px solid #E6E0D2;background:#F6F3EA;font-family:var(--display);font-size:13px">
      <b>Triptych</b><span class="muted" style="font-size:12px">${esc(label||"")}</span><span style="flex:1"></span>
      <a href="${esc(url)}" target="_blank" rel="noopener" style="color:var(--teal);text-decoration:none;font-size:12px" title="open in a full browser tab">⧉ new tab</a>
      <span onclick="tripClose()" title="close (Esc)" style="cursor:pointer;font-size:15px;padding:0 4px;color:#6B655A">✕</span></div>
    <iframe src="${esc(url)}" style="flex:1;border:0;width:100%;background:#fff" loading="eager"></iframe></div>`;
  m.style.display="block";document.body.style.overflow="hidden";return false}
function tripClose(){const m=document.getElementById("tripmodal");if(m){m.style.display="none";m.innerHTML=""}document.body.style.overflow=""}
document.addEventListener("keydown",e=>{if(e.key==="Escape")tripClose()});
...
REQUIRED = [
    ...
    "function tripOpen(",          # Triptych popup modal opener (2026-07-11)
    "function tripClose(",         # Triptych popup modal closer
