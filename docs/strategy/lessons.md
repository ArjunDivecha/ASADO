...
## Paper-book levels are never results (settled 2026-07-13, the "3-Sharpe incident")
- **Rule:** a portfolio number computed at lag 0 (signal "executed" at the close it was computed from), at a 1d hold, or at local index closes is a MEASUREMENT construct, not a strategy result. Reporting such a level without a lag/hold/venue decomposition is a reporting defect. The combiner book printed Sharpe 3.27 under lag-0/1d; the same book is 1.06 at lag-1/1d and 0.97 at lag-1/5d, with harness deflated Sharpe NEGATIVE (-0.10).
- **Mandatory check:** any experiment book must state its execution convention in the PRD (lag, hold, venue/clock); any gross Sharpe > 1.5 on index-space books triggers a lag/hold decomposition BEFORE results leave the session. The cost-law retraction (2026-07-13) removed transaction-cost GATES - it did not license ignoring execution physics (the clock, asynchronous closes, same-close execution).
- Incident record: experiments/2026_07_dispersion_throttle/results/RESULTS.md correction block; ledger ME_20260713_002 methodology_status.
