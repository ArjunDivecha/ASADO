...
gates:
  ...
  - id: G3
    ...
    must_assert: "backfill driver exits 0; all four INV4 clauses pass (corr>=0.95 vs a2, per-year +/-0.012 vs a6, pre-2024 mean +/-0.001 of a6, post-2024 mean<0); family_ic_nightly populated for every (family, roster); gate-state artifact written with network_spillover parked/1 (AUTHOR AMENDMENT 3: June 2026 book-roster IC was genuinely positive, +0.093 after May -0.137 — verified from backfilled data; the state machine was correct and the original parked/0 expectation was the authoring error)"
...
ledger:
  ...
  blockers: []
  lessons:
    - "G3 executed 2026-07-14 (Arjun approved): INV4 all four clauses PASS on first honest run (corr 0.9657, 0 offending years, pre-mean delta 0.00087, flip reproduces); first run FAILED on the authored parked/0 expectation - adjudicated against data (June 2026 book-roster IC +0.093), AUTHOR AMENDMENT 3 set expected=parked/1. Authored point-expectations about live data must be data-verified before binding."
    - "G4 executed: idempotent (1181->1181 rows, status refreshed). Live finding: June 2026 positive on all three families - one more positive month fires the first re-arm signal (2026-08-01)."
