...
- `regime/` — the Regime Conditioning Value Test (PRD-driven, H1/H2/H3): a standalone research sandbox testing whether deterministic macro regimes improve T2 factor IC and portfolio Sharpe. Current committed verdict (H1 PASS, H2 PASS, H3 FAIL on rolling-window win rate) plus the `regime_loop/` Loop Pilot autonomous-optimization harness that tried to push H3 over the line are documented in the Backlog; not wired into the nightly loop.
- `regime_ew/`, `regime_factor_selection/` — separate regime-conditioning experiments with documented null/no-go results; see the Backlog.
...
- `regime/` + `regime_loop/` (Regime Conditioning Value Test) is a **partial / inconclusive** standalone research result, not wired into the nightly loop. The PRD (`regime/PRD.md`) pre-registers three hypotheses: H1 regime persistence ≥0.75, H2 ≥30% of T2 factors significant after BH-FDR, H3 Sharpe Δ ≥0.2 AND ≥60% of rolling 5y windows beating the unconditional baseline. The current committed test (`regime/run_regime_test.py`) uses a 4-regime taxonomy (Crisis/Late-cycle/Expansion/Transition), `3MRet` forward returns, BH-FDR α=0.20, and a full-period `bt_full` for rolling-window Sharpe. The committed verdict (`regime/regime.md`, superseding the all-fail `regime2.md` v2 baseline): H1 PASS (0.793), H2 PASS (21/52, 40.4%), H3 FAIL — Sharpe Δ 0.451 passes the ≥0.2 bar but the rolling-window win rate is 47.2% (< 60%), driven by 2022 inflation-shock contamination of OOS windows. The PRD decision framework routes H1+H2-pass/H3-fail to "try alternative conditioning (sizing/exposure caps) before ML." The `regime_loop/` Loop Pilot autonomous-optimization harness (`looppilot.config.json`, `program.md`, `strategy.md`, 13 iteration reports under `output/reports/`) ran 13 retire iterations (`loop_status: error`, `all_pass: 0`) without crossing the H3 rolling-window threshold. No dedicated page because the test is a partial result that stopped short of production; revisit if a v2 conditioning design is pre-registered or the harness is resumed.
