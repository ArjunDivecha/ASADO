...
def main() -> int:
    ...
    if not args.skip_bloomberg:
        results.append(run_step("T2: daily Bloomberg pull (live blpapi)",
                                "collect_t2_bloomberg.py", ["--daily"], log_file, conda=True))
    else:
        print("\n  (T2 daily Bloomberg pull skipped — reusing existing workbook)")
    results.append(run_step("T2: build daily master", "build_t2_master_daily.py", [], log_file))
    results.append(run_step("T2: normalize daily", "t2_normalize_daily.py", [], log_file))
    results.append(run_step("T2: daily benchmarks", "build_benchmark_rets_daily.py", [], log_file))
    results.append(run_step("T2: daily factor returns", "t2_optimizer_daily.py", [], log_file))

    # ── GDELT daily chain ─────────────────────────────────────────────────
    if not args.t2_only and not args.skip_gdelt:
        results.append(run_step("GDELT: normalize daily", "gdelt_normalize_daily.py", [], log_file))
        ...
        results.append(run_step("GDELT: daily factor returns", "gdelt_optimizer_daily.py", [], log_file))

    # ── DuckDB daily tables + graph ───────────────────────────────────────
    if not args.t2_only and not args.skip_db:
        results.append(run_step("DB: build daily panels", "build_daily_panels.py", ["--rebuild", "--no-backup"], log_file))
    if not args.t2_only and not args.skip_neo4j:
        results.append(run_step("Graph: Neo4j refresh", "setup_neo4j.py", [], log_file))

    # ── Alpha-Hunting Loop (Layer 1) ──────────────────────────────────────
    # Chained here ON PURPOSE (2026-06-10): the standalone 06:45 launchd loop
    # job used to run BEFORE this pipeline finished (~08:40), so detectors
    # scanned yesterday's tables. Running the loop as the last stage
    # guarantees it always sees the data this run just built.
    if not args.t2_only and not args.skip_db and not args.skip_loop:
        results.append(run_step("Loop: Layer 1 (dislocations + brief)",
                                "loop/loop_daily_job.py", [], log_file))

    # ── summary ───────────────────────────────────────────────────────────
    elapsed = time.time() - t_start
    print(f"\n{'='*60}\n  DAILY UPDATE SUMMARY ({elapsed/60:.1f} min)\n{'='*60}")
    for r in results:
        print(f"  [{r['status']:>6}] {r['name']:<42} {r['elapsed']:.1f}s")
