...
"""
=============================================================================
SCRIPT NAME: gdelt_support.py
=============================================================================

INPUT FILES:
- None (utility module; individual functions accept file paths as arguments)

OUTPUT FILES:
- None (utility module; functions write files when called by pipeline scripts)
...
DESCRIPTION:
Support library for GDELT data processing. Provides utility functions for:
- Fetching GDELT master file lists and GKG CSV archives via HTTP
- Parsing GDELT v2 format data (tone, locations, themes, URLs)
- Loading and building GDELT country lookup tables from multiple sources
- Writing JSON output files

External URLs fetched by ensure_support_files():
...
This module is imported by GDELT pipeline scripts and is not run directly.

DEPENDENCIES:
- Python 3.x (standard library: csv, io, json, re, time, urllib, zipfile, pathlib)

USAGE:
    from gdelt_support import (
        load_gkg_urls_by_date, iter_gkg_rows_from_zip_bytes,
        ensure_support_files, load_domain_country_lookup,
        write_json, fetch_bytes, ...
    )

NOTES:
- Cached support files are stored in the directory passed to ensure_support_files().
- Country overrides are defined in the COUNTRY_OVERRIDES dict (e.g. RB -> Serbia).
=============================================================================
"""
