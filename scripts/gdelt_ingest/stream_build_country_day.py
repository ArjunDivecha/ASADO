...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="Number of parallel threads for downloading GKG ZIP files within a single day.",
    )
...
def main() -> None:
    ...
    # ── Download all GKG ZIPs in parallel and process as they finish ──────
    def _download(url: str) -> tuple[str, bytes | None, str | None]:
        try:
            return url, fetch_bytes(url), None
        except Exception as exc:
            return url, None, describe_fetch_error(exc)
    ...
    with ThreadPoolExecutor(max_workers=args.fetch_workers) as pool:
        ...
        for future in as_completed(futures):
            url, payload, error_message = future.result()
