...
def build(as_of: str | None = None) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    try:
        ...
        for country in T2_UNIVERSE:
            ...
            # combiner_scores_daily (COMBINER_RIDGE_DAILY_V1) is a Ridge fit on FORWARD
            # returns. It is still computed here for the price_state_surface 'live_signal'
            # row (a NON-allowlisted table the outcome-blind Discovery Lab cannot read —
            # price_state_surface is not in surface_loader.ALLOWED_SURFACES). Red-team C1
            # (2026-06-26) removed it from source_freshness_json below, which IS allowlisted
            # into the Lab snapshot; it must never go back into the freshness blob.
