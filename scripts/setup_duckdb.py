...
Tables created:
  ...
  - unified_panel:    VIEW union of all five tables
...
def create_indexes(con: duckdb.DuckDBPyConnection):
    ...
    for table in ["t2_master", "external_factors", "extended_factors", "gdelt_panel", "imf_factors"]:
...
def main():
    ...
    for f in [T2_CSV, EXTERNAL_PQ, EXTENDED_PQ, GDELT_CSV, IMF_PQ]:
        if not f.exists():
            print(f"MISSING: {f}")
            sys.exit(1)
